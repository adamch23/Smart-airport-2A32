...
void Dialog_Stats::MakeStat()
{
    ...
    /***** Axe des ordonnées *****/
    ui->plot->yAxis->setRange(0,200);//echelle
...
void Dialog_Stats::statistiquesadam(QVector<double>* ticks,QVector<QString> *labels)
{
    QSqlQuery qry;
    int i=0;
    qry.exec("SELECT CODEDEVOL FROM VOLS");
    while (qry.next())
    {
        QString CODEDEVOL = qry.value(0).toString();
        i++;
        *ticks<<i;
        *labels <<CODEDEVOL;
    }
}
void Dialog_Stats::MakeStatadam()
{
    /***** Background *****/
    QLinearGradient gradient(0, 0, 0, 400);
    gradient.setColorAt(0, QColor(255, 255, 255));
    ...
    A->setStackingGap(1);
    /***** Couleurs*****/
    A->setName("Les transits  selon les codes des vols");
    A->setPen(QPen(QColor(255, 0, 0).lighter(120)));
    A->setBrush(QColor(39, 39, 39));
    /***** Axe des abscisses *****/
    QVector<double> ticks;
    ...
    ui->plot->xAxis->setTickLabelRotation(60);
    ui->plot->xAxis->setSubTicks(false);
    ui->plot->xAxis->setLabel("CODEDEVOL");
    ui->plot->xAxis->setTickLength(0, 4);
    ui->plot->xAxis->setRange(0, 8);
    ui->plot->xAxis->setBasePen(QPen(Qt::black));
    ui->plot->xAxis->setTickPen(QPen(Qt::black));
    ...
    ui->plot->yAxis->setRange(0,200);//echelle
    ui->plot->yAxis->setPadding(5);
    ui->plot->yAxis->setLabel("TRANSIT");
    ui->plot->yAxis->setBasePen(QPen(Qt::black));
    ui->plot->yAxis->setTickPen(QPen(Qt::black));
    ui->plot->yAxis->setSubTickPen(QPen(Qt::black));
    ui->plot->yAxis->grid()->setSubGridVisible(true);
    ...
    ui->plot->yAxis->grid()->setSubGridPen(QPen(QColor(130, 130, 130), 0, Qt::DotLine));
    QVector<double> PlaceData;
    QSqlQuery q1("SELECT TRANSIT FROM VOLS");
    while (q1.next())
    {
        int  nbr_fautee = q1.value(0).toInt();
        PlaceData<< nbr_fautee;
