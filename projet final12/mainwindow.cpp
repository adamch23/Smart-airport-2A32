...
{
    Smtp* smtp = new Smtp("adam.chemengui@esprit.tn",ui->mail_pass->text(), "smtp.gmail.com");
    connect(smtp, SIGNAL(status(QString)), this, SLOT(mailSent(QString)));

    if( !files.isEmpty() )
        smtp->sendMail("adam.chemengui@esprit.tn", ui->rcpt->text() , ui->subject->text(),ui->msg->toPlainText(), files );
    else
        smtp->sendMail("adam.chemengui@esprit.tn", ui->rcpt->text() , ui->subject->text(),ui->msg->toPlainText());
}
...
void DuarteCorporation::MainWindow::update_label()
{
    data=A.read_from_arduino();
ui->label_12avion->setText("Mouvement non détecté");
    if(data=="1")

      {

        ui->label_12avion->setText("Mouvement détecté !!"); // si les données reçues de arduino via la liaison série sont égales à 1
...
void DuarteCorporation::MainWindow::on_pushButton_2_clicked()
{
    QString reference=ui->le_refislem->text();
    int numero=ui->le_numislem->text().toInt();
    int etat=ui->le_etatislem->text().toInt();

    int larg=ui->l_largislem->text().toFloat();
    int longeur=ui->le_longislem->text().toFloat();



Piste p(reference,numero,etat,larg,longeur);
bool test=p.ajouterislem();

if (test)
{

     ui->tab_pistesislem->setModel(p.afficherislem());
...
void DuarteCorporation::MainWindow::on_update_2_clicked()
{
    ...
    /*
     ...
     int numero=ui->modif_num->text().toInt();
         ...
         QString reference=ui->modif_refislem->text();
         int numero=ui->modif_numislem->text().toInt();
         int etat=ui->modif_etatislem->text().toInt();

         int larg=ui->modif_larg->text().toInt();
         int longeur=ui->modif_longeurislem->text().toInt();

       Piste  p(reference,numero,etat,larg,longeur);

         bool test=p.modifierislem();
         if(test)
         {
             //actualiser pour l'affichage
             ui->tab_pistesislem->setModel(p.afficherislem());
...
void DuarteCorporation::MainWindow::on_pushButton_4_clicked()
{
    int numero= ui->le_refsuppislem->text().toInt();
    bool test=p.supprimerislem(numero);
    if (test)
    {
...
void DuarteCorporation::MainWindow::on_butt_rech_clicked()
{
    QString findText;
          QString text = ui->recherche->text();
      Piste p ;
      QTableView* table=ui->tab_pistesislem;
          if (text.isEmpty()) {
              ...
              findText = text;
              p.rechercheislem(table,findText.toUInt());


  }
}

void DuarteCorporation::MainWindow::on_pushButton_8_clicked()
{
    ui->tab_pistesislem->setModel(p.triislem());

}

void DuarteCorporation::MainWindow::on_refrech_clicked()
{
    ui->tab_pistesislem->setModel(p.afficherislem());

}

void DuarteCorporation::MainWindow::on_pushButton_excel_clicked()
{
    QTableView *table;
     table = ui->tab_pistesislem;
...
void DuarteCorporation::MainWindow::on_pushButton_9_clicked()
{
    QString strStream;
                        QTextStream out(&strStream);

                        const int rowCount = ui->tab_pistesislem->model()->rowCount();
                        const int columnCount = ui->tab_pistesislem->model()->columnCount();
                        ...
                        out << "<thead><tr bgcolor=#f0f0f0> <th>Numero</th>";
                        for (int column = 0; column < columnCount; column++)
                            if (!ui->tab_pistesislem->isColumnHidden(column))
                                out << QString("<th>%1</th>").arg(ui->tab_pistesislem->model()->headerData(column, Qt::Horizontal).toString());
                        out << "</tr></thead>\n";
                        ...
                        for (int row = 0; row < rowCount; row++) {
                            ...
                            for (int column = 0; column < columnCount; column++) {
                                if (!ui->tab_pistesislem->isColumnHidden(column)) {
                                    QString data = ui->tab_pistesislem->model()->data(ui->tab_pistesislem->model()->index(row, column)).toString().simplified();
                                    out << QString("<td bkcolor=0>%1</td>").arg((!data.isEmpty()) ? data : QString("&nbsp;"));
                                }
                            }
                            out << "</tr>\n";
...
void DuarteCorporation::MainWindow::on_Send_clicked()
{
    smtp = new Smtp("gestion.pistes.contact@gmail.com" , "nkqjcmhcwbxdwfzd", "smtp.gmail.com",465);
       ...
       smtp->sendMail("contact",ui->a_mailislem->text(),ui->objet_mailislem->text(),msg);
